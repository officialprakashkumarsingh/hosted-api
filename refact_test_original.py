...
def test_all_models_comprehensive():
    """Test all models with comprehensive streaming output"""
    print("🚀 COMPREHENSIVE MODEL TESTING WITH WEBSCOUT")
    print("=" * 80)
    ...
    for i, model in enumerate(client.models.list(), 1):
        print(f"\n{'='*60}")
        print(f"🔄 Testing {i}/{len(client.models.list())}: {model}")
        print('='*60)
        ...
        try:
            ...
            print(f"\n\n✅ SUCCESS")
            print(f"📊 {chunk_count} chunks, {len(content)} chars, {elapsed:.2f}s")
    ...
    print('='*80)
    print(f"✅ Working: {len(working_models)}/{len(client.models.list())}")
    print(f"❌ Failed: {len(failed_models)}/{len(client.models.list())}")
...
if __name__ == "__main__":
    # Test the original example first
