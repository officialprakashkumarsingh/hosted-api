...
def test_model_streaming(client, model_name, timeout_seconds=10):
...
def test_all_models():
    ...
    print(f"📋 Total models to test: {len(Refact.AVAILABLE_MODELS)}")
    ...
    for i, model in enumerate(Refact.AVAILABLE_MODELS, 1):
        print(f"\n🔄 Progress: {i}/{len(Refact.AVAILABLE_MODELS)}")
        success, result = test_model_streaming(client, model)
    ...
    print('='*80)
    print(f"✅ Working models: {len(working_models)}/{len(Refact.AVAILABLE_MODELS)}")
    print(f"❌ Failed models: {len(failed_models)}/{len(Refact.AVAILABLE_MODELS)}")
